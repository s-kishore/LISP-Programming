...
; -- Integer  interp - if the line item is fillable
; -- false   interp - if the line item is not fillable

; TEMPLATE:
; maybeinteger-fn : MaybeInteger -> ??
; (define (maybeinteger-fn value)
;   (cond
;     [(integer? value)...]
;     [(false? value)...]))
;_______________________________________________________________________________
...
(define BOOK4 (make-book 0874999344 "Fire1" "DAK" "INDIA" 4000 3500
                         3 (make-reorders true 1 5) 90))

(define BOOK3-AFT-DLV (make-book 0874512344 "Fire" "DAK" "INDIA" 4000 3500
                         3 (make-reorders true 4 5) 87))

(define BOOK4-AFT-DLV (make-book 0874999344 "Fire1" "DAK" "INDIA" 4000 3500
                         8 (make-reorders false 0 0) 90))
...
(define 3-BOOK-INVENTORY-134
  (cons BOOK1 (cons BOOK3 (cons BOOK4 empty))))

(define NO-REORDER-INVENTORY
  (cons BOOK1 (cons BOOK4-AFT-DLV empty)))

(define 3-BOOK-INVENTORY-134-AFT-DELV
  (cons BOOK1 (cons BOOK3-AFT-DLV (cons BOOK4-AFT-DLV empty))))
...
(define (inventory-potential-profit invtry)
  (foldr
   + 0 (map profit-from-book-copies invtry)))
...
(define (inventory-total-volume invtry)
  (foldr
   + 0 (map volume-of-book-copies invtry)))
...
(define (price-for-line-item invtry lineitem1)
  (foldr
   (lambda (book1 price)
     (if (= (book-isbn book1) (line-item-isbn lineitem1))
         (price-of-line-item-helper book1 (line-item-isbn lineitem1)
                                    (line-item-quantity lineitem1))
         price))
   false
   invtry))
...
;TEST CASES:
(begin-for-test
  (check-equal? (price-for-line-item 3-BOOK-INVENTORY LINEITEM-AVAILABLE2)
                4000
...
(define (price-of-line-item-helper book1 isbn quantity)
...
(define (fillable-now? order invtry)
  (andmap 
   (lambda (lineitem1)
     (is-book-in-stock lineitem1 invtry))
   order))
...
(define (calc-days-till-fillable order invtry)
  (max (if (integer? (calc-days-till-fillable-helper (first order) invtry))
           (calc-days-till-fillable-helper (first order) invtry)
           NOT-FILLABLE)
       (if (empty? (rest order))
           0
           (calc-days-till-fillable (rest order) invtry))))
...
;    GIVEN: the Inventory 
;  RETURNS: the inventory after receiving deliveries for the day
; EXAMPLES: Refer test cases
; STRATEGY: Structural Decomposition on book1: Book

(define (inventory-after-deliveries invtry)
  (map 
   ; Inventory -> Inventory
   ;    Given: a inventory
   ;  Returns: a inventory after reducing the number of days in re-order by 1
   ; Strategy: Structural Decomposition on invtry : Inventory
   (lambda (book1)
     (if (reorder-present? (book-reordrst book1))
         (reduce-delivery-days book1)
         book1)) invtry))

;_______________________________________________________________________________
; reduce-delivery-days Book -> Book 
;    GIVEN: a book
;  RETURNS: the book after reducing the nextshipment days in reorder by one if 
;           a reorder is present
; EXAMPLES: Refer test cases
; STRATEGY:  Structural Decomposition on book1: Book

(define (reduce-delivery-days book1)
  (make-book (book-isbn book1)
             (book-title book1)
             (book-author book1)
             (book-publshr book1)
             (book-uprice book1)
             (book-ucost book1)
             (calc-book-copies (book-copies book1) (book-reordrst book1))
             (reduce-delivery-days-helper (book-reordrst book1))
             (book-cuft book1)))
...
;_______________________________________________________________________________
; calc-book-copies NonNegInt -> Reoreders 
;    GIVEN: the number of copies
;  RETURNS: the number of copies if nextshipment was received today 
; EXAMPLES: Refer test cases
; STRATEGY: Structural Decomposition on reorder1: Reorders

(define (calc-book-copies copies reorder1)
  (cond [(= 1 (reorders-nxtshpmnt reorder1)) 
         (+ copies (reorders-newcopies reorder1))]
        [else copies]))
...
;_______________________________________________________________________________
; reduce-delivery-days-helper Reorders -> Reoreders 
;    GIVEN: a reorder
;  RETURNS: the reorder after a day
; EXAMPLES: Refer test cases
; STRATEGY:  Structural Decomposition on reorder1: Reorders

(define (reduce-delivery-days-helper reorder1)
  (cond [(= 1 (reorders-nxtshpmnt reorder1))
         (make-reorders false 0 0)]
         [else (make-reorders true 
               (- (reorders-nxtshpmnt reorder1) 1)
               (reorders-newcopies reorder1))]))

;TEST CASES:
(begin-for-test
  (check-equal? (inventory-after-deliveries 3-BOOK-INVENTORY-134) 
                3-BOOK-INVENTORY-134-AFT-DELV 
                "Inventory after deliveries Test: the number of days to
                 next delivery should reduce by 1 & copies must be increased")
  
  (check-equal? (inventory-after-deliveries NO-REORDER-INVENTORY)
                NO-REORDER-INVENTORY
                "Inventory after deliveries Test: since there is no 
                reorder the inventory will be returned back")
  )
;_______________________________________________________________________________
...
(define (inventory-after-order invtry order1)
   (map
     ; Book Order -> Book
     ;    GIVEN: a book and an order
     ;  RETURNS: the inventory after the order has been filled.
     ; EXAMPLES: see test case below
    (lambda (book1) (for-book book1 order1)) invtry))

;_______________________________________________________________________________
; for-book : Book Order -> Book
;    GIVEN: a book and an order
;  RETURNS: the inventory after the order has been filled.
; EXAMPLES: Refer test case
; STRATEGY: Higher Order Function Composition

(define (for-book book1 order1)
  (foldr
   ;LineItem -> Book
     ;    GIVEN: a book and an order
     ;  RETURNS: the inventory after the order has been filled.
     ; EXAMPLES: see test case below
   (lambda (lineitem1 rest) 
     (if (check-isbn book1 lineitem1)
         (rebuild-book book1 lineitem1)
         rest)) book1 order1))
 
;_______________________________________________________________________________
; check-isbn : Book LineItem -> Book
;    GIVEN: a book and a lineitem
;  RETURNS: true if the isbn of the book matches with the one in the lineitem
; EXAMPLES: Refer test case
; STRATEGY: Structural Decomposition on book1: Book

 (define (check-isbn book1 lineitem1)
   (check-isbn-helper (book-isbn book1) lineitem1))

;_______________________________________________________________________________
; check-isbn-helper : ISBN LineItem -> Book
;    GIVEN: isbn and a lineitem
;  RETURNS: true if the isbn matches with the one in the lineitem
; EXAMPLES: Refer test case
; STRATEGY: Structural Decomposition on lineitem1: LineItem
 
 (define (check-isbn-helper isbn lineitem1)
   (= isbn (line-item-isbn lineitem1)))

;_______________________________________________________________________________
; rebuild-book : Book LineItem -> Book
;    GIVEN: a book and a lineitem
;  RETURNS: a new book after reorder has been rebuilt after receiving shipment
; EXAMPLES: Refer test case
; STRATEGY: Structural Decomposition on book1: Book
 
 (define (rebuild-book book1 lineitem1)
   (make-book (book-isbn book1)    (book-title book1)  (book-author book1) 
              (book-publshr book1) (book-uprice book1) (book-ucost book1)
              (copies-aft-order (book-copies book1) lineitem1)
              (book-reordrst book1) (book-cuft book1)))
;_______________________________________________________________________________
; copies-aft-order : NonNegInt LineItem -> NonNegInt
;    GIVEN: the number of copies and a lineitem
;  RETURNS: the number of copies after receiving shipment
; EXAMPLES: Refer test case
; STRATEGY: Structural Decomposition on lineitem1: LineItem
 
 (define (copies-aft-order copies lineitem1)
   (- copies (line-item-quantity lineitem1)))
