...
; -- "east"  -- Interp: the StatefulToy<%> is moving towards right canvas border
; -- "west"  -- Interp: the StatefulToy<%> is moving towards left canvas border
...
#; (define (lot-fn lot) 
   (cond
      ...
      [(else (...
                 ...
                 (lot-fn (rest lot)))]))
;_______________________________________________________________________________
; A KeyEvent is of scalar data and can be one of
; - "s"                  interp : add a new square toy to the world with its 
;                                  center located at the centre of the target.
; - "c"                  interp : add a new circular toy to world with its
;                                  center located at the centre of the target.
; - Any other KeyEvent   interp : Ignore

;_______________________________________________________________________________
; A MouseEvent is of scalar data and can be one of 
; -- "button-down"         interp: selects the square toy if the mouse position
;                                  is within the square toy, else no action
; -- "drag"                interp: drags the square toy if the toy is already
;                                  selected, else no action 
; -- "button-up"           interp: unselects a toy if the toy is already
;                                  selected, else no action
; -- Any other MouseEvent  interp: Ignore

;_______________________________________________________________________________
; A ColorString is a scalar data which be one of the following string
...
;_______________________________________________________________________________
; CLASS: World% implements StatefulWorld<%> 
...
;  speed      - Represents the speed the square StatefulToy<%> should move 
;  toys       - A list containing both square and circle StatefulToy<%>
;
...
(define World%
  (class* object% (StatefulWorld<%>)
    ...
    (define/public (on-tick)
      (new World%
           ...
           [toys (map
                  ...
                  ; Given: a StatefulToy<%>
                  ; Returns: the StatefulToy<%> that should follow after a tick.
                  (lambda (toy)
                    (send toy on-tick))
                  toys)]))
    ...
    ; world-after-unclick Integer Integer -> Void
    ;    Given: the x and y coordinate of the mouse unclick event 
    ;   Effect: updates this StatefulWorld<%> to the state that it should be 
    ;           after mouse button up event
    ...
    ; create-sqr-toy:  -> Void
    ;   EFFECT: Add a new square StatefulToy<%> with its center same as target's
    ;            to the StatefulWorld<%>
    ...
    ; create-circ-toy:  -> Void
    ;   EFFECT: Add a new circle StatefulToy<%> with its center same as target's
    ;            to the  StatefulWorld<%>
    ...
    ; on-draw: -> Scene
    ; Returns : a scene potraying this StatefulWorld<%> on it.
    ...
    (define/public (on-draw)
      (local
        ...
        (foldr
         ...
         ; Given: a StatefulToy<%> and a scene
         ; Returns: the StatefulToy<%> placed on the given scene
         (lambda (toy scene)
    ...
    ; Returns : the ListOfStatefulToy<%> in the current StatefulWorld<%>
...
; INTERPRETATION: 
; x     - The x coordinate of the center of the square StatefulToy<%> 
; y     - The y coordinate of the center of the square StatefulToy<%>
; dir   - represents the direction in which the square StatefulToy<%> is moving
...
(define SquareToy%
  (class* object% (StatefulToy<%>)
    ...
    ;   Returns: a Scene like the given one, but with this StatefulToy<%> on it
    ...
    ;   Returns: returns the new x coordinate of this StatefulToy<%> ater moving
    ...
    ;   Returns: the new direction in which the StatefulToy<%> shold travel
    ...
    ; Returns : the color of the StatefulToy<%> 
    ...
    ; toy-x -> Int 
    ; Returns : the x coordinate of the center of the StatefulToy<%>
    ...
    ; toy-y -> Int 
    ; Returns : the y coordinate of the center of the StatefulToy<%>
    ...
    ;is-toy-equal?: StatefulToy<%> -> Boolean
    ;   Given: a square StatefulToy<%> 
    ; Returns: true iff the current square StatefulToy<%> and the given square 
    ;          StatefulToy<%> are equal
    ;Examples: Refer test cases
    ;Strategy: Function Composition
...
;INTERPRETATION:
; x       - The x coordinate of the center of the circle StatefulToy<%> 
; y       - The y coordinate of the center of the circle StatefulToy<%>
...
(define CircleToy%
  (class* object% (StatefulToy<%>)
    ...
    ; determine-color: ColorString NonNegInt -> ColorString 
    ;   Returns: ColorString of the circle StatefulToy<%> after the tick
    ;   Example: (send CIRCLE1 determine-color GREEN ONE) -> GREEN
    ; Startegy : Cases on color : ColorString
    
    (define/public (determine-color color sec-cnt)
      (cond [(and (string=? GREEN color) (= sec-cnt FOUR)) RED]
            [(and (string=? RED color) (= sec-cnt FOUR)) GREEN]
            [else color]))

    ; calc-sec-cnt: NonNegInt -> NonNegInt 
    ;   Returns: Zero iff the count has already reached 4 else adds one to it
    ...
    ; add-to-scene: Scene -> Scene 
    ;   Returns: the scene with the circle StatefulToy<%> of the specified color
    ;            on it
    ...
    ;   Given: a circle StatefulToy<%> 
    ; Returns: true iff the current circle StatefulToy<%> and the given circle 
    ;          StatefulToy<%> are equal
    ;Examples: Refer test cases
    ;Strategy: Function Composition
...
;   Given: a x and y coordinate of the new square StatefulToy<%> to be created 
;          and its speed
...
;   Given: a x and y coordinate of the new circle StatefulToy<%> to be created
; Returns: a green circle StatefulToy<%> with its location in the coordinates 
;          given
...
(define (is-world-equal? w1 w2)
  (and (equal? (send w1 target-x) (send w2 target-x))
       ...
       (andmap 
        ...
        ;   Given: the 2 StatefulToy<%> to be compared
        ; Returns: true iff the two worlds are equal, else false
        (lambda (toy1 toy2)
