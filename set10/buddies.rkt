       ...
       (lambda (square-tgt) 
         (when (and
           ...
           (begin
             ...
             (send square-tgt add-buddy square-src)))) 
       toys))))
...
(define SquareToy% 
  (class* object% (StatefulToy<%> SquareToyExtras<%>)
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;; toy-color : -> SquareToyColorString
    ...
    (define/public (intersects-helper-fn other-x other-y)
      (and (<= (abs (- y other-y)) SQUARE-SIDE)
           (<= (abs (- x other-x)) SQUARE-SIDE)))
...
(define test-unselected-squaretoy-buddy2 
  (new SquareToy% 
       ...
       [color RED][moving? true]))  
(define test-unselected-squaretoy4 
  (new SquareToy% 
       [x 100][y 100][e SQUARE-SIDE][m-x 95][m-y 95][selected? false]
       [buddies (list test-selected-squaretoy-buddy22)]
       [color RED][moving? true])) 
...
(begin-for-test
  ...
  (send test-unselected-squaretoy intersects-helper-fn 100 100)
  ...
  (send test-unselected-squaretoy add-buddy test-unselected-squaretoy)
  ...
  (check-equal?
   ...
   "the square toy should not be selected after a 
   button-up occurs in the selection area.")  
  (send world-for-intersection check-intersections test-unselected-squaretoy4)
  (check-equal?
  ...
  (check-equal?
   (send test-unselected-squaretoy-buddy2 drag-buddy 110 120 100 100)
   (void))
  
   (send test-selected-squaretoy-buddy1 on-mouse 120 130 "drag")
      (check-equal?
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Constants for World class test: 
...
(begin-for-test
  ...
  (send world-on-tick-test on-mouse
        (/ CANVAS-WIDTH 2) (/ CANVAS-WIDTH 2) "button-up")
  (check-equal?
   (send  world-on-tick-test target-selected?)
  ...
  (check-equal?
   (send world-for-intersection mouse-evt-target-world 5 50 100 100 1 1 
                                                       true "drag")
    (void)
    "The toys intersect with each other")
...
(begin-for-test
 ;; squares with buddies
  (local
    ((define squaretoy-on-buddy-test1 (new SquareToy% [x (+ TARGET-X-START 12)] 
                     [y (+ TARGET-Y-START 12)] 
                     [m-x (+ TARGET-X-START 12)] 
                     [m-y (+ TARGET-Y-START 12)] 
                     [buddies empty]
                     [moving? true]))
     (define squaretoy-on-buddy-test2 (new SquareToy% [x (+ TARGET-X-START 20)] 
                     [y (+ TARGET-Y-START 20)] 
                     [m-x (+ TARGET-X-START 20)] 
                     [m-y (+ TARGET-Y-START 20)] 
                     [buddies empty] [selected? false] [color GREEN])))
    (define squaretoy-on-buddy-test3 (new SquareToy% [x (+ TARGET-X-START 52)] 
                    [y (+ TARGET-Y-START 52)] 
                    [m-x (+ TARGET-X-START 52)]
                    [m-y (+ TARGET-Y-START 52)] 
                    [buddies (list squaretoy-on-buddy-test1)] 
                    [selected? false] [color GREEN][moving? true]))
    ; button-down and drag on square with buddy
    (send squaretoy-on-buddy-test1 on-mouse (+ TARGET-X-START 25) 
                                            (+ TARGET-Y-START 25) "drag")
    (send squaretoy-on-buddy-test3 on-mouse (+ TARGET-X-START 55)
                                            (+ TARGET-Y-START 55) "button-down")
    (send squaretoy-on-buddy-test3 on-mouse (+ TARGET-X-START 65)
                                            (+ TARGET-Y-START 65) "drag")
    (send world1 check-intersections squaretoy-on-buddy-test3)
    ;;button-up mouse event
    (send squaretoy-on-buddy-test3 on-mouse (+ TARGET-X-START 55)
                                            (+ TARGET-Y-START 55) "button-up")
    
    ; square color
    (check-equal? (send squaretoy-on-buddy-test1 toy-color) RED)
    
    ; square get-buddy
    (check-equal? (send squaretoy-on-buddy-test1 get-buddy) empty)
    
    ; square color
    (check-equal? (send squaretoy-on-buddy-test2 toy-color) GREEN)))